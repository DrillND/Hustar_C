...
{
   int motorSpeed = 40;
   int blackValueLeft, blackValueRight;
   int whiteValueLeft, whiteValueRight;
   int boundaryValueLeft, boundaryValueRight;
   ...
   while(TRUE)
   {
      ...
      {
         blackValueLeft = getColorReflected(sensorColorLeft);
         blackValueRight = getColorReflected(sensorColorRight);
         move(motorSpeed, motorSpeed, MSEC_0_50);

         whiteValueLeft = getColorReflected(sensorColorLeft);
         whiteValueRight = getColorReflected(sensorColorRight);
         move(0, 0, MSEC_0_50);
   ...
   boundaryValueLeft = (blackValueLeft + whiteValueLeft) / 2;
   boundaryValueRight = (blackValueRight + whiteValueRight) / 2;
   ...
   while(TRUE)
   {
      if(getColorReflected(sensorColorLeft) > boundaryValueLeft && getColorReflected(sensorColorRight) > boundaryValueRight)
      {
         move(motorSpeed, motorSpeed, MSEC_0_05);
      }
      else if(getColorReflected(sensorColorLeft) > boundaryValueLeft && getColorReflected(sensorColorRight) < boundaryValueRight)
      {
